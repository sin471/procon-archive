...
class Solver:
    ...
    def _predict(self, temperature: List[List[int]]) -> List[int]:
        ...
        for i_in in range(self.N):
            ...
            print(f"# measure i={i_in} y=0 x=0", flush=True)

            sample_size = 10
            measured_values = [
                self.judge.measure(i_in, 0, 0) for _ in range(sample_size)
            ]
            measured_mean = sum(measured_values) / sample_size
            ...
            for i_out, pos in enumerate(self.landing_pos):
                diff = abs(temperature[pos.y][pos.x] - measured_mean)
                if diff < min_diff:
                    min_diff = diff
                    estimate[i_in] = i_out
        return estimate
